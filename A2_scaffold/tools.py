...
#------------检测hostile/prompt injection----------------
def _detect_instruction_in_free_text(text):
    """Detect text that tries to act like system/tool instructions."""
    low = text.lower()

    # Tool-output impersonation
    tool_output_markers = [
        "check_referral_criteria returned",
        "get_referral returned",
    ...
    for marker in tool_output_markers + system_instruction_markers:
        if marker in low:
            return marker
...
def check_referral_criteria(specialty, referral_id):
    ...
    instruction = _detect_instruction_in_free_text(text)
